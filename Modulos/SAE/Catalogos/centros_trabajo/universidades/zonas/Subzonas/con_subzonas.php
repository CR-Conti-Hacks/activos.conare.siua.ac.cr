<?php
	...
	$Id_CT				= (isset($_GET['Id_CT'])) ? $_GET['Id_CT'] : ''; //Parámetro de Centro Trabajo
	$Id_Uni				= (isset($_GET['Id_Uni'])) ? $_GET['Id_Uni'] : ''; //Parámetro de Universidad
	...
	/************************   SQL PAGINACION  ********************************/
	/***************************************************************************/
	$sql ="SELECT COUNT(Id_SZ) AS Cant_Registros
			FROM tab_sub_zona
			WHERE Activo_SZ='1' AND Id_Zon_SZ=".$Id_Zon;
				
	$bs_nom_sz = (isset($_GET['bs_nom_sz'])) ? $_GET['bs_nom_sz'] : '';
	if ($bs_nom_sz != "") {
		$sql.=" AND Nombre_SZ like '%" . $bs_nom_sz ."%'";
	}
	$bs_dim_sz = (isset($_GET['bs_dim_sz'])) ? $_GET['bs_dim_sz'] : '';
	if ($bs_dim_sz != "") {
		$sql.=" AND Diminutivo_SZ like '%" . $bs_dim_sz ."%'";
	}	
	...
	/***************************************************************************/
	if($or_nom_sz!=""){
		$sql.=" ORDER BY ".$or_nom_sz.' '.$or_nom_sz_tipo;
	}elseif($or_dim_sz!=""){
		$sql.=" ORDER BY ".$or_dim_sz.' '.$or_dim_sz_tipo;
	}
...
<div id="Titulo_Adentro">
	<span class="Texto_Titulo" onclick="ActivaTextoTitulo();"><?= $texto['TITULO_Consultar_SZ'] ?></span>
</div>
    ...
    <table class="width500 centrado" >		
		<tr>
			...
			<td class="fondo_gris_claro2">
				<input type="text" name="bs_nom_sz" id="bs_nom_sz"  maxlength="60" value="<?=$bs_nom_sz?>"
				onKeyPress="Buscar(
								event,
								'Modulos/SAE/Catalogos/centros_trabajo/universidades/zonas/con_subzonas.php',
								'pagina;inicio;Id_CT;Id_Uni;Id_Zon;Id_SZ;bs_nom_ct;bs_dim_ct;or_nom_ct;or_dim_ct;or_nom_ct_tipo;or_dim_ct_tipo;bs_nom_uni;bs_dim_uni;or_nom_uni;or_dim_uni;or_nom_uni_tipo;or_dim_uni_tipo;bs_nom_zon;bs_dim_zon;or_nom_zon;or_dim_zon;or_nom_zon_tipo;or_dim_zon_tipo;bs_nom_sz;bs_dim_sz;or_nom_sz;or_dim_sz;or_nom_sz_tipo;or_dim_sz_tipo;',
								'<?=$pagina?>'+';'+'<?=$inicio?>'+';'+
								<?=$Id_CT?>+';'+
								<?=$Id_Uni?>+';'+
								<?=$Id_Zon?>+';'+
		...
		<tr>
			...
			<td class="fondo_gris_claro2">
				<input type="text" name="bs_dim_sz" id="bs_dim_sz"  maxlength="60" value="<?=$bs_dim_sz ?>"
				onKeyPress="Buscar(
								event,
								'Modulos/SAE/Catalogos/centros_trabajo/universidades/zonas/con_subzonas.php',
								'pagina;inicio;Id_CT;Id_Uni;Id_Zon;Id_SZ;bs_nom_ct;bs_dim_ct;or_nom_ct;or_dim_ct;or_nom_ct_tipo;or_dim_ct_tipo;bs_nom_uni;bs_dim_uni;or_nom_uni;or_dim_uni;or_nom_uni_tipo;or_dim_uni_tipo;bs_nom_zon;bs_dim_zon;or_nom_zon;or_dim_zon;or_nom_zon_tipo;or_dim_zon_tipo;bs_nom_sz;bs_dim_sz;or_nom_sz;or_dim_sz;or_nom_sz_tipo;or_dim_sz_tipo;',
...
<div class="component">
	...
	<table class="centrado font09 width80P ">		
		<tr class="centrado">
			<thead>
					<?php if(in_array("1073",$_SESSION['Permisos'])){ ?>
					<th>
						<?=$texto['Ver_Detalle']?>
					</th>
					<?php } ?>
					...
					<th>
						<a onClick="javascript:Ordenar('Modulos/SAE/Catalogos/centros_trabajo/universidades/zonas/Subzonas/con_subzonas.php',
												   'or_nom_sz',
												   'Nombre_SZ',
												   'or_nom_sz_tipo',
												   'pagina;inicio;Id_CT;Id_Uni;Id_Zon;Id_SZ;bs_nom_ct;bs_dim_ct;or_nom_ct;or_dim_ct;or_nom_ct_tipo;or_dim_ct_tipo;bs_nom_uni;bs_dim_uni;or_nom_uni;or_dim_uni;or_nom_uni_tipo;or_dim_uni_tipo;bs_nom_zon;bs_dim_zon;or_nom_zon;or_dim_zon;or_nom_zon_tipo;or_dim_zon_tipo;bs_nom_sz;bs_dim_sz;',
													'<?=$pagina?>'+';'+'<?=$inicio?>'+';'+
													<?=$Id_CT?>+';'+
													<?=$Id_Uni?>+';'+
													<?=$Id_Zon?>+';'+
				...
				<?php if(in_array("1074",$_SESSION['Permisos'])){ ?>
					<th><a><?=$texto['Modificar']?></a></th>
				<?php } ?>
				
				<?php if(in_array("1075",$_SESSION['Permisos'])){ ?>
					<th><a><?=$texto['Personas']?></a></th>
				<?php } ?>
								
				<?php if(in_array("1077",$_SESSION['Permisos'])){ ?>
					<th><a><?=$texto['Consultar_Mapa_CT']?></a></th>
				<?php } ?>
				
				
				<?php if(in_array("1079",$_SESSION['Permisos'])){ ?>
		...
		?>
				<tr>
					<?php if(in_array("1073",$_SESSION['Permisos'])){ ?>
						<td class="centrado">
							<a onclick="MostrarDetalle('Modulos/SAE/Catalogos/centros_trabajo/universidades/zonas/Subzonas/ajax_detalle_subzona.php','Id_SZ;','<?=$res[$i]['Id_SZ']?>;','<?=$i?>');">
					...
					<td class="centrado"><?= $res[$i]['Nombre_SZ']?></td>
					<td class="centrado"><?= $res[$i]['Diminutivo_SZ']?></td>
					<td class="centrado"><?= $res[$i]['Telefono_SZ']?></td>
					...
					<td class="centrado">
						...
						onClick="CargaPaginaMenu(
											    'Modulos/SAE/Catalogos/centros_trabajo/universidades/zonas/Subzonas/mod_subzona.php',
											    'pagina;inicio;Id_CT;Id_Uni;Id_Zon;Id_SZ;bs_nom_ct;bs_dim_ct;bs_nom_uni;bs_dim_uni;bs_nom_zon;bs_dim_zon;bs_nom_sz;bs_dim_sz;or_nom_ct;or_dim_ct;or_nom_uni;or_dim_uni;or_nom_zon;or_dim_zon;or_nom_sz;or_dim_sz;or_nom_ct_tipo;or_dim_ct_tipo;or_nom_uni_tipo;or_dim_uni_tipo;or_nom_zon_tipo;or_dim_zon_tipo;or_nom_sz_tipo;or_dim_sz_tipo;mostrar_efecto;',
												'<?=$pagina?>'+';'+'<?=$inicio?>'+';'+
												<?=$Id_CT?>+';'+<?=$Id_Uni?>+';'+<?=$Id_Zon?>+';'+<?=$res[$i]['Id_SZ']?>+';'+
												document.getElementById('bs_nom_ct').value+';'+
												document.getElementById('bs_dim_ct').value+';'+
												document.getElementById('bs_nom_uni').value+';'+
												document.getElementById('bs_dim_uni').value+';'+
					...
					<td class="centrado">
						<img src='img/SAE/ver_derechos.png' alt='<?=$texto['Consultar_Derechos']?>' 
							onclick="CargaPaginaMenu('Modulos/SAE/Catalogos/centros_trabajo/universidades/zonas/Subzonas/personas_x_subzonas.php','pagina;inicio;Id_CT;Id_Uni;Id_Zon;Id_SZ;or_nom_ct;or_nom_ct_tipo;bs_nom_ct;or_dim_ct;or_dim_ct_tipo;bs_dim_ct;bs_nom_uni;bs_dim_uni;or_nom_uni;or_dim_uni;or_nom_uni_tipo;or_dim_uni_tipo;bs_nom_zon;bs_dim_zon;or_nom_zon;or_nom_zon_tipo;or_dim_zon;or_dim_zon_tipo;bs_nom_sz;bs_dim_sz;or_nom_sz;or_nom_sz_tipo;or_dim_sz;or_dim_sz_tipo;mostrar_efecto;',
								'<?=$pagina?>;<?=$inicio?>;'
								+<?=$Id_CT?>+';'
								+<?=$Id_Uni?>+';'
								+<?=$Id_Zon?>+';'
								...
								+document.getElementById('or_nom_sz_tipo').value+';'
								+document.getElementById('or_dim_sz').value+';'
								+document.getElementById('or_dim_sz_tipo').value+';1;');"
						/>
					</td>
					<?php } ?>
					...
					<td class="centrado">
						<img src='img/SAE/eliminar.png' alt='<?=$texto['Eliminar']?>' onClick="MostrarVentana(
																												this,
																												1,
																												'Modulos/SAE/Catalogos/centros_trabajo/universidades/zonas/Subzonas/eli_subzona.php',
																												'pagina;inicio;Id_SZ;Id_Zon;Id_Uni;Id_CT;bs_nom_ct;bs_dim_ct;bs_nom_uni;bs_dim_uni;bs_nom_zon;bs_dim_zon;bs_nom_sz;bs_dim_sz;or_nom_ct;or_dim_ct;or_nom_uni;or_dim_uni;or_nom_zon;or_dim_zon;or_nom_sz;or_dim_sz;or_nom_ct_tipo;or_dim_ct_tipo;or_nom_uni_tipo;or_dim_uni_tipo;or_nom_zon_tipo;or_dim_zon_tipo;or_nom_sz_tipo;or_dim_sz_tipo;',
																												'<?=$pagina?>;<?=$inicio?>;<?=$res[$i]['Id_SZ']?>;<?=$Id_Zon?>;<?=$Id_Uni?>;<?=$Id_CT?>;<?=$bs_nom_ct?>;<?=$bs_dim_ct?>;<?=$bs_nom_uni?>;<?=$bs_dim_uni?>;<?=$bs_nom_zon?>;<?=$bs_dim_zon?>;<?=$bs_nom_sz?>;<?=$bs_dim_sz?>;<?=$or_nom_ct?>;<?=$or_dim_ct?>;<?=$or_nom_uni?>;<?=$or_dim_uni?>;<?=$or_nom_zon?>;<?=$or_dim_zon?>;<?=$or_nom_sz?>;<?=$or_dim_sz?>;<?=$or_nom_ct_tipo?>;<?=$or_dim_ct_tipo?>;<?=$or_nom_uni_tipo?>;<?=$or_dim_uni_tipo?>;<?=$or_nom_zon_tipo?>;<?=$or_dim_zon_tipo?>;<?=$or_nom_sz_tipo?>;<?=$or_dim_sz_tipo?>;')"
						>
					</td>
					<?php } ?>
				</tr>
			...
			<div class="Paginacion gris">
				<?php if ($pagina !=1){ ?>
					<a onClick="javascript:CargaPaginaMenu(
															'Modulos/SAE/Catalogos/centros_trabajo/universidades/zonas/Subzonas/con_subzonas.php',
															'pagina;inicio;Id_CT;Id_Uni;Id_Zon;'+
															'bs_nom_ct;bs_dim_ct;'+
															...
															'bs_nom_sz;bs_dim_sz;'+
															'or_nom_sz;or_nom_sz_tipo;'+
															'or_dim_sz;or_dim_sz_tipo;'
															,
															'<?=$pagina - 1 ?>;<?=$inicio - $TAMANO_PAGINA ?>;<?=$Id_CT?>;<?=$Id_Uni?>;<?=$Id_Zon?>;'+
															document.getElementById('bs_nom_ct').value+';'+
															document.getElementById('bs_dim_ct').value+';'+
															document.getElementById('or_nom_ct').value+';'+
															document.getElementById('or_nom_ct_tipo').value+';'+
...
<!-- ****************************************************************************************** -->
	...
	<br/>
	<div class="centrado">
		<button class="boton"  onclick="CargaPaginaMenu('Modulos/SAE/Catalogos/centros_trabajo/universidades/zonas/Subzonas/agr_subzona.php','pagina;inicio;Id_Zon;Id_Uni;Id_CT;bs_nom_ct;bs_dim_ct;bs_nom_uni;bs_dim_uni;bs_nom_zon;bs_dim_zon;bs_nom_sz;bs_dim_sz;or_nom_ct;or_dim_ct;or_nom_uni;or_dim_uni;or_nom_zon;or_dim_zon;or_nom_sz;or_dim_sz;or_nom_ct_tipo;or_dim_ct_tipo;or_nom_uni_tipo;or_dim_uni_tipo;or_nom_zon_tipo;or_dim_zon_tipo;or_nom_sz_tipo;or_dim_sz_tipo;mostrar_efecto;',
								'<?=$pagina?>;<?=$inicio?>;'
								+<?=$Id_Zon?>+';'
								+<?=$Id_Uni?>+';'
								+<?=$Id_CT?>+';'
								...
								+document.getElementById('or_nom_sz_tipo').value+';'
								+document.getElementById('or_dim_sz_tipo').value+
								';1;');"><?=$texto['TITULO_Agregar_SZ']?></button>
								
		<button class="boton"  onclick="CargaPaginaMenu('Modulos/SAE/Catalogos/centros_trabajo/universidades/zonas/con_zonas.php','pagina;inicio;Id_Zon;Id_Uni;Id_CT;bs_nom_ct;bs_dim_ct;bs_nom_uni;bs_dim_uni;bs_nom_zon;bs_dim_zon;or_nom_ct;or_dim_ct;or_nom_uni;or_dim_uni;or_nom_zon;or_dim_zon;or_nom_ct_tipo;or_dim_ct_tipo;or_nom_uni_tipo;or_dim_uni_tipo;or_nom_zon_tipo;or_dim_zon_tipo;mostrar_efecto;',
								'<?=$pagina?>;<?=$inicio?>;'
								+<?=$Id_Zon?>+';'
								+<?=$Id_Uni?>+';'
								+<?=$Id_CT?>+';'
