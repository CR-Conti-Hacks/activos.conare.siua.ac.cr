<?php
    ...
    */
	...
	/************************   SQL PRINCIPAL   ********************************/
	/***************************************************************************/
	$sql ="SELECT Id_Per,Cedula_Per,Tipo_Identificacion_Per,Nombre_Per,Apellido1_Per,Apellido2_Per FROM tab_personas WHERE Activo_Per = '1' AND Id_Per != 1 AND Id_Per!=2";
    /*if($bs_nom_tip_tel  != "" ){
		$sql.=" AND Nombre_Tip_Tel like '%".$bs_nom_tip_tel."%'";
	}
...
<div id="Titulo_Adentro">
	<span class="Texto_Titulo" onclick="ActivaTextoTitulo();"><?= $texto['TITULO_Consultar_Personas']  ?></span>
</div>
...
<!-- ***************************************************************************************-->
    <table class="centrado width40P">		
            <tr class="centrado">
                <thead>
					<?php if(in_array("1578",$_SESSION['Permisos'])){ ?>
					<th>
						<?=$texto['Ver_Detalle']?>
					</th>
					<?php }?>
            ...
            <tr>
				<?php if(in_array("1578",$_SESSION['Permisos'])){ ?>
					<td class="centrado">
						<a onclick="MostrarDetalle('Modulos/SAE/Personas/ajax_detalle_persona.php','Id_Usuario;','<?=$res[$i]['Id_Per']?>;','<?=$i?>');">
							<img name="img_detalle<?=$i?>" id="img_detalle<?=$i?>" alt="<?= $texto['Ver_Detalle']?>" src="img/SAE/mostrar_detalle.png"  />
						</a>		
					</td>
				<?php }?>
				<td><?=$res[$i]['Id_Per']?></td>
				<td><?=$res[$i]['Cedula_Per']?></td>
				<td><?=$res[$i]['Tipo_Identificacion_Per']?></td>
				<td><?=$res[$i]['Nombre_Per']?></td>
				<td><?=$res[$i]['Apellido1_Per']?></td>
				<td><?=$res[$i]['Apellido2_Per']?></td>
