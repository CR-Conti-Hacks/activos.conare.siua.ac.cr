<?php
	...
	$sql = "SELECT 	DISTINCT	
					...
					h.Id_Per_Usu_HMA,
					h.Fecha_HMA,
					h.Modificaciones_HMA,
                    p.Nombre_Per,
                    p.Apellido1_Per,
                    p.Apellido2_Per
					
			FROM tab_sae_inv_historial_modificacion_activos AS h
			INNER JOIN (tab_usuarios AS u) ON (h.Id_Per_Usu_HMA = u.Id_Per_Usu) 
			INNER JOIN (tab_personas AS p) ON (u.Id_Per_Usu = p.Id_Per)
			WHERE Id_Act_HMA = ".$Id_Act;

	$sql.=" ORDER BY Id_HMA DESC ";   
...
var valores_parametros = 	
	...
	<div id="Titulo_Adentro">
		<span class="Texto_Titulo" onclick="ActivaTextoTitulo();">Historial Activo:</span><br>
		<hr>
	...
	<table class="centrado width90P tabConsultaActivos">		
        ...
        ?>
        			<tr class="centrado" >
        				<td class="centrado"><?= $Diminutivo_ACON ?>-<?=$res_principal[$i]['Id_Act_HMA']?></td>
        				<td class="centrado"><?=$res_principal[$i]['Fecha_HMA']?></td>
        				<td class="centrado"><?=$res_principal[$i]['Modificaciones_HMA']?></td>
