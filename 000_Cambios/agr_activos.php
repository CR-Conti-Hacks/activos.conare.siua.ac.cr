...
<!-- ****************************************************************************************** -->
	<table class="centrado width40P" >
		...
		<tr id="filaMotivoCambioubicacion" style="display: none;">
		...
		<tr>
			<td>Responsable:</td>
			<td>
				<select id="Id_Res_Act" name="Id_Res_Act">
					<option value="0">[Seleccione]</option>
					<?php
						$sql ="SELECT Id_Res,Nombre_Res FROM tab_Responsables ORDER BY Nombre_Res ASC";
						$res = seleccion($sql);
						for($i=0;$i<count($res);$i++){
					?>
						<option value="<?=$res[$i]['Id_Res']?>"><?=$res[$i]['Nombre_Res']?></option>
					<?php
						}
					?>
				</select>
		...
		<tr>
			<td>Custodio:</td>
			<td>
				<select id="Id_Cus_Act" name="Id_Cus_Act">
					<option value="0">[Seleccione]</option>
					<?php
						$sql ="SELECT Id_Cus,Nombre_Cus FROM tab_Custodios ORDER BY Nombre_Cus ASC ";
						$res = seleccion($sql);
						for($i=0;$i<count($res);$i++){
					?>
						<option value="<?=$res[$i]['Id_Cus']?>"><?=$res[$i]['Nombre_Cus']?></option>
					<?php
						}
					?>
				</select>
...
<script>
		...
		$('#Tiempo_Garantia_Act').darkTooltip({
			  opacity:0.9,
			  size: 'large',
			  animation:'flipIn',
			  gravity:'west', // 
